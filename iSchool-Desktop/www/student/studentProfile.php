...
include_once('../dbConnection.php');
 ...
 if(isset($_REQUEST['updateStuNameBtn'])){
  if(($_REQUEST['stuName'] == "")){
   ...
   $stuIMG = $_REQUEST["stuImgOrg"];
   $stu_image = $_FILES['stuImg']['name']; 
   $stu_image_temp = $_FILES['stuImg']['tmp_name'];
   $img_folder = '../image/stu/'. $stu_image; 
   move_uploaded_file($stu_image_temp, $img_folder);
   if($img_folder == '../image/stu/'){
    $sql = "UPDATE student SET stu_name = '$stuName', stu_occ = '$stuOcc', stu_img = '$stuIMG' WHERE stu_email = '$stuEmail'";
   } else {
...
?>
 <div class="col-sm-6 mt-5">
  <form class="mx-5" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
      <!-- <label for="stuImgOrg">Upload Image</label> -->
      <input type="hidden" class="form-control-file" value="<?php if(isset($stuImg)) {echo $stuImg;} ?>" id="stuImgOrg" name="stuImgOrg">
    </div>
